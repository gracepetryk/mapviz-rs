...
- **Layers:** trait `Layer { fn prepare(&mut self, frame: &mut Frame); }`. A layer emits `Shape`s (a `geo` geometry + a `Style`) into the `Frame` — it never issues GPU calls or touches a render pass, which is what keeps `wgpu` out of the scene logic. Built-ins live in `mapviz-layers` (e.g. the general-purpose `ShapeLayer`); users implement the same trait for custom layers.
...
- The contract between scene and renderer is **data, not a trait**: a `Frame` of ordered `Shape`s (`mapviz-core`), which `mapviz-render` consumes. Layers fill the `Frame`; the renderer owns everything about turning it into pixels. There is no `Backend` trait today — `mapviz-render` is concretely wgpu, and the public API talks to it directly (see Guiding principles for when that would change).
- A `Shape` is a `geo::Geometry<f32>` plus a `Style` (fill / stroke / stroke-width / point-size). The scene vocabulary is standard `geo` geometry — points, lines, polygons, and their `Multi*`/collection forms — not a bespoke primitive enum. A GPU-free tessellator in `mapviz-core` (`tessellate_shape`) turns each `Shape` into flat draw instances: `Polygon`/`MultiPolygon` → an earcutr-triangulated fill (plus a stroked outline), `LineString` → stroked segments, `Point` → quad markers. Group many like primitives into a single `Multi*` geometry to have them drawn in one batched/instanced pass; `Frame` order is the render (painter's) order. `mapviz-render` mirrors the flat instances (`QuadInstance`/`LineInstance`/`FillVertex`) with GPU-layout types (`GpuQuad`…, `bytemuck::Pod`) via `From`, so the `bytemuck` casts stay in the render crate and core has no GPU types.
- 3D is the same `Frame`/`Shape` flow with a 3D camera matrix plus a depth attachment; no separate capability negotiation, since wgpu does everything.
...
- `geo` — geometry types (`Point`, `LineString`, `Polygon`, …) that form the scene vocabulary, plus predicates/simplification. **In use.** Only the types are exercised today, so trimming to `geo-types` or `geo` with `default-features = false` is a likely WASM-size win.
